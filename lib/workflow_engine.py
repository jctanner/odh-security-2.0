...
class WorkflowEngine:
    ...
    def _load_workflow_with_includes(self, workflow_name: str, loaded_workflows: set) -> WorkflowDefinition:
        ...
        if workflow_name in loaded_workflows:
            raise ValueError(f"Circular include detected: {workflow_name} is already being loaded")
        ...
        if 'includes' in workflow_data:
            for include_name in workflow_data['includes']:
                try:
                    included_workflow = self._load_workflow_with_includes(include_name, loaded_workflows.copy())
                    ...
                    print(f"Warning: Could not load included workflow '{include_name}': {e}")
                    continue
    ...
    def execute_workflow(self, workflow_name: str, variables: Dict[str, str] = None) -> bool:
    ...
    def _execute_workflow_definition(self, workflow: WorkflowDefinition, variables: Dict[str, str] = None) -> bool:
    ...
    def _execute_step(self, step: WorkflowStep, variables: Dict[str, str], step_num: int, total_steps: int) -> bool:
    ...
    def preview_workflow_variables(self, workflow_name: str, runtime_vars: Dict[str, str] = None) -> Dict[str, str]:
