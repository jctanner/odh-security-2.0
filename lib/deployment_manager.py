...
class DeploymentManager:
    ...
    def deploy_operator(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        if not operator_yaml:
            return DeploymentResult(
                success=False,
                message="Operator YAML file not found"
            )
        
        # Apply operator resources
        result = self._apply_yaml_file(operator_yaml, config.namespace, config.dry_run, verbose)
        
        if result.success and not config.dry_run:
            # Wait for operator to be ready
            operator_ready = self._wait_for_operator_ready(config.namespace, config.wait_timeout, verbose)
    ...
    def deploy_dsci(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        if not dsci_yaml:
            return DeploymentResult(
                success=False,
                message="DSCI YAML file not found"
            )
        
        # Apply DSCI resources
        result = self._apply_yaml_file(dsci_yaml, config.namespace, config.dry_run, verbose)
        
        if result.success and not config.dry_run:
            # Wait for DSCI to be ready
            dsci_ready = self._wait_for_dsci_ready(config.namespace, config.wait_timeout, verbose)
            if not dsci_ready:
                result.success = False
    ...
    def deploy_dsc(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        if not dsc_yaml:
            return DeploymentResult(
                success=False,
                message="DSC YAML file not found"
            )
        
        # Apply DSC resources
        result = self._apply_yaml_file(dsc_yaml, config.namespace, config.dry_run, verbose)
        
        if result.success and not config.dry_run:
            # Wait for DSC to be ready
            dsc_ready = self._wait_for_dsc_ready(config.namespace, config.wait_timeout, verbose)
            if not dsc_ready:
                result.success = False
    ...
    def deploy_full(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        if config.operator_enabled:
            ...
            if not operator_result.success:
                ...
                combined_result.message = f"Operator deployment failed: {operator_result.message}"
        ...
        if config.dsci_enabled:
            ...
            if not dsci_result.success:
                combined_result.success = False
                combined_result.message = f"DSCI deployment failed: {dsci_result.message}"
                combined_result.duration = time.time() - start_time
                return combined_result
    ...
    def undeploy_operator(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        return self._delete_yaml_file(operator_yaml, config.namespace, config.dry_run, verbose)
    
    def undeploy_dsci(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        return self._delete_yaml_file(dsci_yaml, config.namespace, config.dry_run, verbose)
    
    def undeploy_dsc(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        return self._delete_yaml_file(dsc_yaml, config.namespace, config.dry_run, verbose)
    
    def undeploy_full(self, config: DeploymentConfig, verbose: bool = False) -> DeploymentResult:
        ...
        if config.operator_enabled:
            ...
            if not operator_result.success:
                print(f"Warning: Operator undeployment failed: {operator_result.message}")
    ...
    def _create_namespace(self, namespace: str, verbose: bool = False) -> DeploymentResult:
        ...
        if self._namespace_exists(namespace):
            return DeploymentResult(success=True, message=f"Namespace '{namespace}' already exists")
    ...
    def _apply_yaml_file(self, yaml_file: str, namespace: str, dry_run: bool = False, verbose: bool = False) -> DeploymentResult:
    ...
    def _delete_yaml_file(self, yaml_file: str, namespace: str, dry_run: bool = False, verbose: bool = False) -> DeploymentResult:
    ...
    def _run_kubectl_command(self, cmd: List[str], verbose: bool = False) -> DeploymentResult:
    ...
    def _wait_for_operator_ready(self, namespace: str, timeout: int, verbose: bool = False) -> bool:
        """Wait for operator to be ready
    ...
    def _wait_for_dsci_ready(self, namespace: str, timeout: int, verbose: bool = False) -> bool:
        """Wait for DSCI to be ready
    ...
    def _wait_for_dsc_ready(self, namespace: str, timeout: int, verbose: bool = False) -> bool:
        """Wait for DSC to be ready
