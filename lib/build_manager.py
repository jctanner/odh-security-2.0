...
class BuildConfig:
    ...
    def __post_init__(self):
        ...
        if self.custom_registry and not all(
            [self.registry_url, self.registry_namespace, self.registry_tag]
        ):
            raise ValueError(
                "Custom registry requires registry_url, registry_namespace, and registry_tag"
            )

        if self.push_image and not self.image:
            raise ValueError(
                "Cannot push image without building image (image=True required)"
            )
...
class BuildManager:
    ...
    def _determine_make_target(self, config: BuildConfig) -> str:
        ...
        if config.manifests_only:
            if config.local:
                return (
                    "get-manifests-local-branch"
                    if config.use_branch
                    else "get-manifests-local"
                )
            else:
                return (
                    "get-manifests-fork-branch"
                    if config.use_branch
                    else "get-manifests-fork"
                )
        elif config.image:
            if config.custom_registry:
                if config.local:
                    return (
                        "image-build-custom-registry-local-branch"
                        if config.use_branch
                        else "image-build-custom-registry-local"
                    )
                else:
                    return (
                        "image-build-custom-registry-fork-branch"
                        if config.use_branch
                        else "image-build-custom-registry-fork"
                    )
            else:
                if config.local:
                    return (
                        "image-build-local-branch"
                        if config.use_branch
                        else "image-build-local"
                    )
                else:
                    return (
                        "image-build-fork-branch"
                        if config.use_branch
                        else "image-build-fork"
                    )
        else:
    ...
    def _execute_make(
        self, target: str, env_vars: Dict[str, str], verbose: bool = False
    ) -> BuildResult:
        ...
        try:
            ...
            return BuildResult(
                ...
                stderr=stderr,
                image_name=(
                    self._extract_image_name(stdout) if "image" in target else None
                ),
    ...
    def get_build_status(self) -> Dict[str, any]:
        ...
        status = {
            ...
            "makefile_exists": os.path.exists(
                os.path.join(self.operator_dir, "Makefile")
            ),
            "dockerfile_exists": os.path.exists(
                os.path.join(self.operator_dir, "Dockerfile")
            ),
            "manifests_dir_exists": os.path.exists(
                os.path.join(self.operator_dir, "opt", "manifests")
            ),
        }
        ...
        if os.path.exists(manifests_dir):
            status["manifest_count"] = len(
                [f for f in os.listdir(manifests_dir) if f.endswith(".yaml")]
            )
        else:
